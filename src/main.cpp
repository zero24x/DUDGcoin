...
#if defined(NDEBUG)
# error "dudgcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "dudgcoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexBest->nHeight <= 16000) {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1;
    }
	 if(pindexBest->nHeight <= 21300) {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;
	}
	else if(pindexBest->nHeight <= 34800) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 4.5;
       }
	...
	else if(pindexBest->nHeight <= 91000) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 3;
      } 
    else if(pindexBest->nHeight <= 112000) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1.5;
      } 
	else if(pindexBest->nHeight <= 142000) {
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 0.25;
      } 
       ...
       nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 2;
				...
				nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8) * 1/4;
			}
		}	
	else {

...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("DudgCoin"))
        return false;
...
{
    // Convert DudgCoin:// to DudgCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("DudgCoin://"))
    {
        uri.replace(0, 12, "DudgCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DudgCoin.lnk";
}
...
{
    return GetAutostartDir() / "DudgCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DudgCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DudgCoin-qt [" + tr("command-line options") + "]                     " + "\n";
